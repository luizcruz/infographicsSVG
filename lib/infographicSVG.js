/*
    ...
    Author: Luiz Cruz
    Year: 2015
    Updated: 2024 - Declarative HTML API; secure DOM construction; soccer, trophy and glove shapes
    ...
    Shape types : humans | box | circle | soccer | trophy | glove
...
(function () {
    ...
    var VALID_TYPES = { humans: true, box: true, circle: true, soccer: true, trophy: true, glove: true };
    ...
    // Returns null when the attribute is absent so render knows to skip the legend.
    function parseLegend(raw) {
        if (!raw) { return null; }
        var s = String(raw).trim();
        if (!s) { return null; }
        return s.split(';').map(function (l) { return l.trim(); });
    }

    // ── HTML helpers for title and legend ─────────────────────────────
    ...
    // Returns a <div> with one swatch+label item per segment that has a non-empty label.
    // Colors are already safeColor-validated by the time they reach here.
    function buildLegend(segments, labels) {
        var wrap = document.createElement('div');
        wrap.style.display = 'flex';
        wrap.style.flexWrap = 'wrap';
        wrap.style.gap = '6px 16px';
        ...
        for (var i = 0; i < segments.length; i++) {
            ...
            swatch.style.background = segments[i][1];
    ...
    function buildGlove(color) {
        // Goalkeeper glove: cuff at bottom, palm in center, four fingers up, thumb right.
        var g = svgEl('g', { fill: color, 'fill-opacity': 0.85, stroke: '#333', 'stroke-width': 0.8 });
        // Wrist cuff
        g.appendChild(svgEl('rect', { x: -7, y: 9,  width: 14, height: 6,  rx: 2 }));
        // Palm
        g.appendChild(svgEl('rect', { x: -8, y: -8, width: 16, height: 19, rx: 4 }));
        // Fingers: pinky, ring, middle, index (left to right)
        g.appendChild(svgEl('rect', { x: -7.5, y: -21, width: 3, height: 15, rx: 1.5 }));
        g.appendChild(svgEl('rect', { x: -4,   y: -23, width: 3, height: 17, rx: 1.5 }));
        g.appendChild(svgEl('rect', { x: -0.5, y: -24, width: 3, height: 18, rx: 1.5 }));
        g.appendChild(svgEl('rect', { x:  3,   y: -22, width: 3, height: 16, rx: 1.5 }));
        // Thumb (right side, slightly angled downward)
        g.appendChild(svgEl('ellipse', { cx: 11, cy: -1, rx: 4.5, ry: 2.5,
            transform: 'rotate(-20,11,-1)' }));
        return g;
    }
    ...
    function render(target, numberOfItems, itemsPerColumn, segments, shapeType, title, legendLabels) {
        ...
        if (shapeType === 'glove')  { svgWidth = 14 * numberOfItems; svgHeight = 11 * numberOfItems; }
        ...
        while (items > 0) {
            ...
            if (countColor < select) {
                ...
                countColor = 1;
                // Guard: never advance past the last segment so remaining items
                // keep the last color rather than causing an out-of-bounds crash.
                if (indexCount < segments.length - 1) { indexCount++; }
            }
            ...
            } else if (shapeType === 'glove') {
                shape = buildGlove(color);
                shape.setAttribute('transform', 'translate(' + (33 * col) + ',' + (44 * row) + ')');
            }
        ...
        // Replace target content — no innerHTML used anywhere.
        while (target.firstChild) {
            target.removeChild(target.firstChild);
        }
        if (title) { target.appendChild(buildTitle(title)); }
        target.appendChild(svg);
        if (legendLabels) {
            var legendEl = buildLegend(segments, legendLabels);
            if (legendEl.hasChildNodes()) { target.appendChild(legendEl); }
        }
    }
    ...
    function initElement(container) {
        ...
        var legend = parseLegend(container.getAttribute('data-legend'));

        if (!items || !perCol || !type || !VALID_TYPES[type] || !segs) {
            container.textContent = '[infographic: invalid configuration]';
            return;
        }
        render(container, items, perCol, segs, type, title, legend);
    }
    ...
    // Guard: if the script loads after the DOM is already ready (e.g. via defer,
    // dynamic injection, or bfcache restore), call initAll() immediately instead
    // of waiting for DOMContentLoaded which may never fire again.
    ...
    // doGraph(numberOfItems, itemsPerColumn, [[pct,color],...], shapeType, targetId[, options])
    // options: { title: string, legend: string[] }  — both optional

    window.doGraph = function (numberOfItems, itemsPerColumn, percentageColorArray, shapeType, targetId, options) {
        ...
        var legend = (options && options.legend) ? options.legend          : null;
        render(target, items, perCol, segs, shapeType, title, legend);
    };
